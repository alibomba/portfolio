...
<header class="header">
        ...
        <form action="sklep" method="get" class="header__search">
            <input autocomplete="off" name="fraza" type="text" class="header__search__input" placeholder="Szukaj...">
            ...
            <div class="header__dropdown header__dropdown--produkty header__dropdown--ulubione<?php if(!$zalogowano){echo '-wylogowano';} ?>">
                <?php
                    if($zalogowano){
                        ...
                        $query = "SELECT produkty.id,produkty.nazwa,produkty.kategoria,produkty.cena FROM produkty,ulubione WHERE ulubione.user_id='$user_id' AND ulubione.id_produktu=produkty.id ORDER BY ulubione.data_dodania DESC LIMIT 5";
            ...
            <div class="header__dropdown header__dropdown--produkty header__dropdown--koszyk<?php if(!$zalogowano){echo '-wylogowano';} ?>">
                <?php
                    if($zalogowano){
                        echo '<div class="header__dropdown__produkty">';
                        $user_id = $_SESSION['user_id'];
                        $query = "SELECT produkty.id,produkty.nazwa,produkty.kategoria,produkty.cena FROM produkty,koszyki WHERE koszyki.user_id='$user_id' AND koszyki.id_produktu=produkty.id ORDER BY koszyki.data_dodania DESC LIMIT 5";
                        if($result = $con->query($query)){
                            $pozycje = $result->fetch_all(MYSQLI_ASSOC);
                            if($result->num_rows>0){
                                foreach($pozycje as $pozycja){
